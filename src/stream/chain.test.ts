...
describe('extractChainContext', () => {
  it('extracts files modified from Write and Edit tool uses', () => {
    const session = makeSession({
      toolUses: [
        { type: 'tool_use', id: 'tu-1', name: 'Write', input: { file_path: '/src/foo.ts' } },
        { type: 'tool_use', id: 'tu-2', name: 'Edit', input: { file_path: '/src/bar.ts' } },
        { type: 'tool_use', id: 'tu-3', name: 'Read', input: { file_path: '/src/baz.ts' } },
      ],
    });
  ...
  it('deduplicates file paths', () => {
    const session = makeSession({
      toolUses: [
        { type: 'tool_use', id: 'tu-1', name: 'Write', input: { file_path: '/src/foo.ts' } },
        { type: 'tool_use', id: 'tu-2', name: 'Write', input: { file_path: '/src/foo.ts' } },
      ],
    });
    const ctx = extractChainContext(session);
  ...
  it('extracts commands from Bash tool uses', () => {
    const session = makeSession({
      toolUses: [
        ...
        { type: 'tool_use', id: 'tu-2', name: 'Bash', input: { command: 'pnpm typecheck' } },
      ],
    });
    const ctx = extractChainContext(session);
  ...
  it('extracts errors from tool results', () => {
    const session = makeSession({
      toolResults: [
        { type: 'tool_result', tool_use_id: 'tu-1', content: 'Success', is_error: false },
        { type: 'tool_result', tool_use_id: 'tu-2', content: 'File not found', is_error: true },
      ],
    });
    const ctx = extractChainContext(session);
    ...
    const assistantMsg: StreamAssistantMessage = {
      ...
      message: {
        content: [
          ...
          { type: 'tool_use', id: 'tu-1', name: 'Write', input: { file_path: '/src/comp.ts' } },
        ],
      },
    };
    const session = makeSession({
  ...
  it('limits key decisions to last 5', () => {
    const events: StreamAssistantMessage[] = Array.from({ length: 10 }, (_, i) => ({
      type: 'assistant' as const,
      message: { content: [{ type: 'text' as const, text: `Decision ${i}` }] },
    }));
