...
function buildStoryTable(
  parsedStories: ReturnType<typeof parsePrd>["stories"],
...
function writeStuckHint(
  ...
  const content = [
    ...
    errorResults.length > 0
      ? errorResults
          .map((e) => `- [${e.tool_use_id}] ${e.content.slice(0, 200)}`)
          .join("\n")
      : "_No error results_",
    "",
    `## Files Modified`,
...
function runStuckDetection(
  ...
  if (
    ...
    if (isZeroProgress)
      reasons.push("No Write/Edit tool uses detected (zero progress)");
    if (isHighErrorRate)
      reasons.push("High error rate (>50% of tool results are errors)");
...
export async function runWorkspace(
  ...
  for (let iteration = 0; iteration < maxIterations; iteration++) {
    if (fs.existsSync(stoppedPath)) {
      emitter.system(
        `[william] Workspace "${workspaceName}" has been stopped. Halting.`,
      );
      normalExit = true;
      break;
    }
    if (fs.existsSync(pausedPath)) {
      emitter.system(
        `[william] Workspace "${workspaceName}" is paused. Halting.`,
      );
      normalExit = true;
      break;
    }
    ...
    if (currentStory === null) {
      emitter.system(
        `[william] All stories complete for workspace "${workspaceName}".`,
      );
      normalExit = true;
      break;
    }
...

    {
      ...
      emitter.dashboardUpdate({
        ...
        storiesSkipped: storyValues.filter((s) => s.passes === "skipped")
          .length,
        cumulativeCostUsd,
        cumulativeInputTokens,
        cumulativeOutputTokens,
        storyAttempts: attempts,
        stuckStatus: !hintExists
          ? "normal"
          : attempts >= 4
            ? "approaching-skip"
            : "hint-written",
    ...
    if (result.storyComplete || result.allComplete) {
      ...
      emitter.system(
        `[william] Story ${currentStory} not yet complete (attempts: ${attempts}).`,
      );
    }
...

    {
      ...
      emitter.dashboardUpdate({
        ...
        storiesSkipped: updatedValues.filter((s) => s.passes === "skipped")
          .length,
        cumulativeCostUsd,
        cumulativeInputTokens,
        cumulativeOutputTokens,
    ...
    if (stuckResult.action === "pause") {
      emitter.system(
        `[william] Stuck detection triggered pause for workspace "${workspaceName}".`,
      );
      normalExit = true;
      break;
    }
