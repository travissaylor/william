...
export function Dashboard({ data, startTime }: DashboardProps) {
  ...
  const stuckColor = data.stuckStatus === 'normal' ? 'green'
    : data.stuckStatus === 'hint-written' ? 'yellow'
    : 'red';

  const stuckLabel = data.stuckStatus === 'normal' ? '● normal'
    : data.stuckStatus === 'hint-written' ? '▲ hint written'
    : '▲ approaching skip';
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="cyan" paddingX={1}>
      <Box>
        <Text bold color="cyan">{data.workspaceName}</Text>
        <Text dimColor>{' │ '}</Text>
      ...
      <Box>
        ...
        <Text dimColor>{' │ '}</Text>
        <Text bold color="white">{formatElapsed(elapsed)}</Text>
        <Text dimColor>{' │ '}</Text>
